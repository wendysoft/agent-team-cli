...
- **跨会话的偏好/约定/踩坑** → Codex 没有文件系统级 memory；它的跨会话记忆载体就是 **AGENTS.md 本身**和对应 skill 文件，规则直接写入本文件对应章节。若 Claude Code 也参与，同步写入 `~/.claude/projects/.../memory/`。
- **用户给出的反馈或纠正** → 先判断是否构成 durable 工作规则（"下次会话/其他 Agent 也应遵守"）。是的话，**立刻**更新 AGENTS.md + CLAUDE.md 对应章节，而不是只在当次任务应用。
